...
export default function UserSkills({ user }: { user: User }) {
  const [skills, setSkills] = useState<string[]>(user.skills || []);
  const [editing, setEditing] = useState(false);
  const [textarea, setTextarea] = useState(skills.join('\n'));

  // Lưu kỹ năng mới
  const handleSave = async () => {
    const newSkills = textarea
      .split('\n')
      .map(s => s.trim())
      .filter(s => s);
    ...
    // Gọi API lưu skills mới lên Firestore
    await fetch("/api/user/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: user.id, skills: newSkills }),
    });
  };

  return (
    <article className="card-interview flex items-start gap-4 w-full p-4 border-black dark:border-white"
    style={{ borderWidth: '1.5px' }}>
      {/* Nút Edit ở góc phải trên, chỉ hiện khi đã có skills và không editing */}
      ...
      <div className="flex-1 w-full">
        <h3 className="font-bold text-lg mb-4">Skills & Endoresments</h3>
        {!editing ? (
          <>
            {skills.length === 0 ? (
              <>
                <p className="text-base text-gray-400 mb-4 italic">
                  Show how you match new opportunities - 50% of hiring managers rely on skills data to recruit candidates.
                </p>
                <button
          ...
          <div className="flex flex-col gap-2">
            <textarea
              ...
              placeholder="Nhập mỗi kỹ năng trên một dòng..."
              autoFocus
            />
